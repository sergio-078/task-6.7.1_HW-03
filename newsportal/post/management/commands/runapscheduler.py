...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        scheduler.add_job(
            my_job,
            # trigger=CronTrigger(day_of_week="mon", hour="21", minute="31", timezone=settings.TIME_ZONE),  # Каждую пятницу в 18:00Every 10 seconds
            trigger=CronTrigger(day_of_week="fri", hour="18", minute="00", timezone=settings.TIME_ZONE),
            # trigger=CronTrigger(timezone=settings.TIME_ZONE),
